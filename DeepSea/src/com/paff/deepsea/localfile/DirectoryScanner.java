...
import com.paff.deepsea.R;
import com.paff.deepsea.util.FileUtils;
import com.paff.deepsea.util.MimeTypes;
import com.paff.deepsea.FileExplorerActivity;

public class DirectoryScanner extends Thread
{
	private static final String TAG = "DSFE_DirScanner";
	...
	public void run()
	{
		...
		if (!cancel)
		{
			...
			Message msg = handler.obtainMessage(FileExplorerActivity.MESSAGE_SHOW_DIRECTORY_CONTENTS);
			msg.obj = contents;
			msg.sendToTarget();
		}
	...
	private void updateProgress(int progress, int maxProgress)
	{
		...
		if ((progress % PROGRESS_STEPS) == 0)
		{
			...
			// Okay, send an update.
			Message msg = handler.obtainMessage(FileExplorerActivity.MESSAGE_SET_PROGRESS);
			msg.arg1 = progress;
	...
	private static void initializeCupcakeInterface()
	{
		try
		{
			formatter_formatFileSize = Class.forName("android.text.format.Formatter").getMethod("formatFileSize",
